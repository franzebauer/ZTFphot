...
| `batch_pipeline.py` | Process a list of targets in sequence, keeping only the final LC parquets and plots for each. Supports two input modes auto-detected by column count: **RA/Dec mode** (2 columns: `ra,dec`) runs the full pipeline including merge; **quadrant mode** (4 columns: `field,ccdid,qid,filtercode`) skips merge, derives the quadrant center from ztfquery field tables for lookup, suppresses target plots (`--no-target`). Run: `python ZTFphot/scripts/batch_pipeline.py targets.txt [--both] [--bands zg zr] [--workers 20]` |
